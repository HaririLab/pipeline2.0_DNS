...
subDir=/mnt/BIAC/munin4.dhe.duke.edu/Hariri/DNS.01/Analysis/All_Imaging/${sub}_rerun3 #pipenotes= Change away from HardCoding later
...
3dcalc -a ${oldSubDir}/antCT/highRes_BrainSegmentationPosteriors1.nii.gz -b ${tmpDir}/resampAseg.nii.gz -expr 'step(step(a-.35)+step(b))' -prefix ${tmpDir}/tmp.test.nii.gz
3dmask_tool -input ${tmpDir}/tmp.test.nii.gz -prefix ${tmpDir}/tmp2.test.nii.gz -dilate_input 1 -1 -fill_holes
3dcalc -a ${oldSubDir}/antCT/highRes_rWarped.nii.gz -b ${tmpDir}/tmp2.test.nii.gz -expr 'a*b' -prefix ${tmpDir}/test.nii.gz
antsRegistrationSyN.sh -d 3 -t r -f ${templateDir}/${templatePre}_Brain.nii.gz -m ${tmpDir}/test.nii.gz -n $threads -o ${antDir}/${antPre}r
...
if [[ ! -f ${antDir}/${antPre}CorticalThicknessNormalizedToTemplate.nii.gz ]];then
	...
	3dcalc -a ${antDir}/${antPre}rWarped.nii.gz -expr 'step(a)' -prefix ${antDir}/${antPre}BrainExtractionMask.nii.gz
	3dcalc -a ${antDir}/${antPre}BrainExtractionMask.nii.gz -b ${antDir}/${antPre}rWarped.nii.gz -expr 'a*b' -prefix ${antDir}/${antPre}ExtractedBrain.nii.gz
