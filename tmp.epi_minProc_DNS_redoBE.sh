...
subDir=/mnt/BIAC/munin4.dhe.duke.edu/Hariri/DNS.01/Analysis/All_Imaging/${subRedo}/ #pipenotes= Change away from HardCoding later
outDir=${subDir}/${task}
tmpDir=${outDir}/tmp
QADir=${subDir}/QA
antDir=${subDir}/antCT
...
if [[ $task == "rest1" && $restPre2 != "not_collected" ]];then
	...
	echo "/mnt/BIAC/munin4.dhe.duke.edu/Hariri/DNS.01/Analysis/Max/scripts/Pipelines/epi_minProc_DNS.sh $sub rest2 $threads >> ${subDir}/QA/LOG.rest2" >> ${outDir}/swarm.rest2
	echo ""
	echo "#########################################################################################################"
